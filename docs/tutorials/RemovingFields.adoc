...
+
TIP: Removing usages may result in errors, syntax or otherwise.
Exercise discretion and fix them.
For example, removing `address` from the `Person` class will require you to modify its constructor.
...
```
You can go through each individual file and manually remove the `address` field.
