...
Note how the diagram shows only how the execution flows _between_ the main component. That is, it does not show details of
the execution path _inside_ each component. By hiding those details, the digram succeeds in informing the reader
about the
overall execution path of a command without overwhelming the reader with too much details. In this tutorial, you aim to
find those omitted details so that you get a more in-depth understanding of the code base.
...
However, that the execution path through a GUI is often somewhat obscure due to various _even-driven mechanisms_
...
image::LogicSequenceDiagram.png[width=1080px, height=476px]
